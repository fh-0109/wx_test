// pages/core/net_card/net_card.js
import { getNetCard } from '../../../utils/api'
//获取应用实例
var app = getApp();
Page({
  data: {
  ...
  sendRequest: function () {
    ...
    var data = {
      'userid': app.openid,
      'card_id': this.data.net_card.card_id,
      'password': this.data.net_card.password
    }

    getNetCard(data).then(res =>{
      console.log(res.data)
      _this.yktRender(res.data.card)
    })
  },

  yktRender: function(card) {
    var _this = this
    var data = card.detail.slice(0, _this.data.count).reverse();
    /*
    * 获取最近消费数据绘制折线图
    **/
    var dict = data;
    ...
    for(var i = 0; i<len; i++) {
      xArr.push(i * spaceX);
      costArr.push(parseFloat(dict[i][3]));
    }
    console.log(card)
    _this.setData({
      dict: data,
    ...
    * 绘制横轴&纵轴&网格线
    */
    _this.drawLineXY(_this.data.options, _this.data.costArr);

    /*
    * 描点连线
    */
    _this.drawPointLine(_this.data.options, _this.data.costArr);
  ...
  drawPointLine: function (options, switchArr) {
    ...
    */
    ...
    for (var i = 0, pLen = pointArr.length; i < pLen; i++) {
      ...
      pointArr[i].detail[3] = parseFloat(pointArr[i].detail[3]);
    }
  ...
  bind: function () {
    if (!this.data.card_id || !this.data.password) {
      app.showErrorModal('账号及密码不能为空', '提醒');
      return false;
    }

    if (this.data.card_id.length != 13) {
      app.showErrorModal('请输入正确卡号', '提醒');
    }
